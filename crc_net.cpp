...
int main()
{
	...
	cout<<"\nRECEIVER SIDE\n";
	cout<<"Enter received Frame length : ";
	int rn;
	cin>>rn;
	...
	if(rn==fs+rs)
    {
        for(i=0;i<fs+rs;i++)
        {
            if(rf[i]!=tf[i])
            {
                cout<<"Message Transmitted From Sender To Receriver Contains Error!\n";
                cout<<"Received message should have been : ";
                for(i=0;i<fs+rs;i++)
                {
                    cout<<tf[i];
                }
                cout<<"  which when divided with the generator polynomial gives Reaminder equal to 0.";
                break;
            }
        }
        if(i==fs+rs)
            cout<<"Message transmitted is correct !!\n";
    }
    else if(rn!=fs+rs)
    {
            cout<<"Message Transmitted From Sender To Receriver Is Incorrect!\n";
            cout<<"Received message should have been : ";
                for(i=0;i<fs+rs;i++)
                {
                    cout<<tf[i];
